...
#define trace(...)
#endif
//multiply everything with 10, now problem reduces to finding solutions of linear diophantine within a given range
...
int main()
{
  ...
  while(t--)
    {
      ll x1,y1,x2,y2;
      x1=inp(); y1=inp(); x2=inp(); y2=inp();
      ll l1,r1,l2,r2;
      l1=ceil(min(x1,x2)/10.0);
      l2=ceil(min(y1,y2)/10.0);
      r1=floor(max(x1,x2)/10.0);
      r2=floor(max(y1,y2)/10.0);
      ll a,b,c;
      a=y2-y1,b=x1-x2;
      c=a*x1+b*y1;
      a*=10; b*=10;
      cout<<getsoln(a,b,c,l1,r1,l2,r2)<<endl;
    }
  return 0 ;
}
